...
def maxPathSum(root):
    ...
    subtree의 정보를 활용해서 root에서 수합/판단하는 Bottom up approach. recursive한 설계이지만 root를 지나는 path를 완성시키는 combine과정과 별개로 중간중간 정답값인 maxval을 업데이트하는 과정이 있다.인상깊었던 트릭은 maxPathDown의 결과를 0과 비교하여 노드를 탈락시키는 것. max(left, right)과 함께 원하는 효과를 낸다.
    ...
    이걸 dp문제처럼 접근하면 left subtree를 더할지 right subtree를 더할지 결정할때 left subtree의 합과 right subtree의 합을 구해야하는데, 이게 최대값일 수 있으니까 저장해놓고 진행한다고 생각할 수 있다. tree의 구조 상 subtree도 온전한 tree라는 점을 주지하기. left subtree와 right subtree의 maxsum이 구해져있다고 가정할때 루트노드에서 이를 어떻게 활용할 것인가?
    '''
    maxval = float('-inf')
    maxPathDown(root)
    return maxval
